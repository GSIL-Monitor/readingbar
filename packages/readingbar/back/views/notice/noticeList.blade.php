		            ...
		            <table class="footable table table-stripped toggle-arrow-tiny default breakpoint footable-loaded" data-page-size="15">
		                ...
		                <tbody>
		                    <tr class="footable-even" style="display: table-row;" v-for="n in notices.data">
		                        ...
		                        <td class="footable-visible">[[n.notice]]</td>
		                        <td class="footable-visible">[[n.url]]</td>
...
var noticesList=new Vue({
	...
	methods:{
		...
		doGetnotices:function(){
			...
			$.ajax({
				url:"{{ url('admin/api/notice/getNotices') }}",
				data:_this.search,
				dataType:"json",
		...
		doDelete:function(){
			if(confirm('是否确认删除！')){
				var _this=this;
				$.ajax({
					url:"{{ url('admin/api/notice/deleteNotice') }}",
