			...
			<div class="col-md-12">
				<div class="ibox float-e-margins">
	                        <div class="ibox-title">
	                            <h5>{{ trans('PointOrder.view_list') }}</h5>
	                        </div>
	                            ...
	                            <div class="table-responsive">
				                  ...
				                  <th>
				                 	<a href="javascript:void(0)" class="control-label order-id" style="margin-bottom: 0px" v-on:click="orderBy('id')">
				                 		...
				                 		{{ trans('PointOrder.columns.id') }}
				                 	</a>
				                  </th>
				                   <th>
				                 	<a href="javascript:void(0)" class="control-label order-order_id" style="margin-bottom: 0px" v-on:click="orderBy('order_id')">
				                 		<i class="fa fa-sort"></i>
				                 		{{ trans('PointOrder.columns.order_id') }}
				                 	</a>
				                  </th>
				                   <th>
				                 	<a href="javascript:void(0)" class="control-label order-member_id" style="margin-bottom: 0px" v-on:click="orderBy('member_id')">
				                 		<i class="fa fa-sort"></i>
				                 		{{ trans('PointOrder.columns.member') }}
				                 	</a>
				                  </th>
				                     <th>
				                 	<a href="javascript:void(0)" class="control-label order-student_id" style="margin-bottom: 0px" v-on:click="orderBy('student_id')">
				                 		<i class="fa fa-sort"></i>
				                 		{{ trans('PointOrder.columns.student') }}
				                 	</a>
				                  </th>
				                  <th>
				                 	<a href="javascript:void(0)" class="control-label order-status" style="margin-bottom: 0px" v-on:click="orderBy('status')">
				                 		<i class="fa fa-sort"></i>
				                 		{{ trans('PointOrder.columns.status') }}
				                 	</a>
				                  </th>
				                  <th>
				                 	<a href="javascript:void(0)" class="control-label order-created_at" style="margin-bottom: 0px" v-on:click="orderBy('created_at')">
				                 		<i class="fa fa-sort"></i>
				                 		{{ trans('PointOrder.columns.created_at') }}
				                 	</a>
				                  </th>
				                  <th>
				                 	<a href="javascript:void(0)" class="control-label order-updated_at" style="margin-bottom: 0px" v-on:click="orderBy('updated_at')">
				                 		<i class="fa fa-sort"></i>
				                 		{{ trans('PointOrder.columns.updated_at') }}
				                 	</a>
				                  </th>
				                  <th>{{ trans('PointOrder.columns.operation') }}</th>
				                </thead>
					                <tbody>
						                 <tr v-if="list" v-for="d in list.data">
						                   ...
						                   <td>[[ d.updated_at ]]</td>
						                   <td>
						                   		<a :href="d.edit" class="btn btn-default">{{ trans('PointOrder.btns.edit') }}</a>
						                   </td>
						                </tr>
					                </tbody>
	                                </table>
		...
		var mainContent=new Vue({
			...
			methods:{
				...
				doGetList:function(){
					var _this=this;
					$.ajax({
						url:"{{ url('admin/PointOrder/getList') }}",
						dataType:"json",
						data:_this.search,
						beforeSend:function(){
