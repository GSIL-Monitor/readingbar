...
namespace Readingbar\Api\Frontend\Guard;
...
class MemberGuard implements StatefulGuard, SupportsBasicAuth
{
    use GuardHelpers;
    ...
    public function checkPromoter(){
    	if(Promotions::where('member_id','=',$this->id())->count()){
    		return true;
    	}else{
    		return false;
    	}
    ...
    //判断用户是否是蕊丁使者
    public function checkRDMessenger(){
    	if(Promotions::where(['member_id'=>$this->id(),'type'=>5])->count()){
    		return true;
    	}else{
    		return false;
    	}
    ...
    //获取用户的推广人id信息
    public function getConnetedPromoter(){
    	if($this->user->pcode){
    		$return=DB::table('promotions as p')
			    		->where(['p.pcode'=>$this->user->pcode])
			    		->first(['p.*']);
    	}else{
    		$return=null;
    	}
    ...
    public function avatar(){
    	if($this->user){
    		if($this->user->avatar){
    			...
    			return url('files/avatar/default_avatar.jpg');
    		}
    	}
    	return url('files/avatar/user-192.png');
    }
    public function unreadMessages(){
    	return DB::table('messages as s')
    		->where(['receiver_del'=>0,'receiver_read'=>0])
    		->whereIn('sendto',[
    				$this->user->cellphone,
    				$this->user->email
    		])
     ...
     * 判断会员是否有买过任意产品
     */
    public function hasBoughtAnyProduct() {
    	return (boolean)DB::table('students as s')
    ...
    public function getFiled($filed){
    	if(isset($this->user[$filed])){
    		return $this->user[$filed];
    	}else{
    		return null;
    	}
    }
